...
def inv_kin(goal_position):
    ...
    cos_theta_2 = (
        x_desired * x_desired
        + y_desired * y_desired
        - robot_params.link_1_length * robot_params.link_1_length
        - robot_params.link_2_length * robot_params.link_2_length
    ) / (2 * robot_params.link_1_length * robot_params.link_2_length)
...
def at_goal(goal_position):
    ...
    # Verify
    difference_norm = math.fabs(
        goal_position[0] - end_effector_position[0]
    ) + math.fabs(goal_position[1] - end_effector_position[1])
        ...
        if sim_interface.start_simulation():
            ...
            while not at_goal(goal_position):
                ...
                desired_joint_angle_rates = (
                    differential_kinematics.get_desired_joint_rate(current_joint_angles)
                )
